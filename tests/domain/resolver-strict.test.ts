...
test("WDET-02 strict: every workflow signal resolves to one exact unsupported kind", async () => {
  const cases: readonly {
  ...
  for (const c of cases) {
    ...
    const r = await resolveStrict(basicEntry({ source: c.source, ...c.entry }), mockCtx(MP, files));
...
test("WDET-02 strict: named local workflow layouts classify in parallel without a network seam call", async () => {
  let networkCalls = 0;
  ...
  const results = await Promise.all(
    entries.map((entry) => {
      ...
      return resolveStrict(entry, ctx);
    }),
  );
...
test("D-106-06 strict: a workflow signal cannot expose a structurally invalid plugin root", async () => {
  const ctx = mockCtx(MP, { [ROOT("./local")]: "dir" });
  const r = await resolveStrict(
