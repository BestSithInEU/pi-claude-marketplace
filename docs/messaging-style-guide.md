...
type PluginNotificationMessage =
  ...
  | PluginAvailableMessage // status: "available";    NO scope (SNM-11); reasons? (OUT-02)
  | PluginRemoteMessage // status: "remote";       NO scope (SNM-11); reasons? (OUT-05 / RSTA-01); ◌ (RSTA-01)
...
- `reasons: readonly Reason[]` REQUIRED on eight variants -- `partially-available | unavailable | upgradable | skipped | failed | manual recovery` (D-15-01) plus the two derived partial-state realized tokens, `partially-installed` (FSTAT-02 / D-66-03) and `partially-upgradable` (FSTAT-04 / D-66-02 / D-66-03), which are modeled on the `upgradable` arm and carry the degradation detail. It is OPTIONAL on four transition variants -- `installed` (SURF-05 / D-63-08), `updated` (WARN-01 / WR-12), `reinstalled` (WARN-01 / WR-09) and `disabled` (ENBL-16 / D-100-07) -- which carry a brace only when their ledger produced a fact worth naming, and render byte-identically to a reasons-less row when it did not. It is OPTIONAL on two not-installed candidate variants as well, `available` (OUT-02) and `remote` (OUT-05 / RSTA-01), where the brace states what an install WOULD do rather than what one did. The five remaining variants -- `uninstalled` and the four `will *` pending tokens (DIFF-02) -- omit the field entirely and cannot acquire one, so `(uninstalled) {up-to-date}` is a compile error. Eight plus six plus five is the whole 19-member `PLUGIN_STATUSES` tuple: the three lists are exhaustive, not illustrative.
